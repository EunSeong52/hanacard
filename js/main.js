...
$(document).ready(function(){
  ...
  // 버튼.click(function(){}); /어떤 일은 함수function(){}
  // 버튼.hasClass('active') /버튼이 active라는 class를 가지고 있는가? 
  // -> 가지고 있다면 true, 가지고 있지 않다면 false가 뜬다.
  ...
  $('.event_box2 .page_act .btn_play').click(function(e){
        e.preventDefault(); // 사이트가 위로 튕겨올라가지 않도록 하는 것
    if(!$('.event_box2 .page_act').hasClass("on")){
      // on을 가지고 있지 않아야 true
      // ! -> not, 부정 -> 아닐 때 true가 됨
...
tab.mouseover(function(){
  let target=$(this); //버튼 4개 중에서 사용자가 마우스를 올린 버튼 그것
  //버튼 4개 중에서 사용자가 마우스를 올린 버튼 그것의 index번호를 추출
  let index=target.index() 
  // console.log(index)
  style.css({opacity:0}) // 1번. 모두 안보이기  
  style.eq(index).css({opacity:1}) // 2번. 사용자가 마우스를 올린 버튼의 index번호에 해당하는 .life_style ul li요소를 보이게 한다.   
})
